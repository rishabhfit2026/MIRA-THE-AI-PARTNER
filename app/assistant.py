...
from .config import get_model, get_openai_api_key
...
def build_system_prompt(state: AssistantState) -> str:
    ...
    return (
        f"You are {profile.assistant_name}, a caring human-like voice assistant for {profile.name}. "
        "Speak naturally, warmly, and clearly. Be concise. "
        "Focus on health routines, sleep, food timing, hydration, exercise, and gentle accountability. "
        "Reply in the same language the user speaks: English, Hindi, or natural Hinglish. "
        "Do not claim medical diagnosis. Encourage professional care for serious symptoms.\n\n"
        f"User profile:\n"
...
def fallback_reply(state: AssistantState, message: str, actions: list[str]) -> str:
    ...
    if has_hindi:
        if any(word in message.lower() for word in ["today", "schedule", "plan"]) or "आज" in message:
            return (
                f"{action_text} आज का routine यह है: wake up {profile.wake_time}, breakfast {profile.breakfast_time}, "
        ...
        return (
            f"{action_text} मैं तुम्हारी sleep, meals, hydration, exercise और reminders manage कर सकती हूं. "
            "मुझे ऐसे बोलो: wake time 06:30 कर दो, sleep time 22:30 कर दो, या आज का plan बताओ."
        ).strip()

    if any(word in message.lower() for word in ["today", "schedule", "plan"]):
        return (
            f"{action_text} Today's routine is wake at {profile.wake_time}, breakfast at {profile.breakfast_time}, "
    ...
    if any(word in message.lower() for word in ["tired", "low energy", "sleepy"]):
        return (
            f"{action_text} You sound low on energy. Start with water, a light walk, and avoid pushing caffeine too late. "
    ...
    if any(word in message.lower() for word in ["hungry", "food", "eat", "meal"]):
        return (
            f"{action_text} Keep your meals regular: breakfast at {profile.breakfast_time}, lunch at {profile.lunch_time}, "
            f"and dinner at {profile.dinner_time}. Aim for protein, fiber, and enough water with each meal."
        ).strip()

    return (
        f"{action_text} I'm tracking your routine and can adjust wake time, sleep time, meals, exercise, and hydration. "
        "Tell me things like 'set my wake time to 06:30' or 'what is my plan today?'"
    ).strip()
